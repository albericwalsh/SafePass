...
def initialize(app_name: str = "app", file_path: str | None = None, to_file: bool | None = None, max_size_mb: int | None = None, retention_days: int | None = None, settings_path: str | None = None):
    ...
    try:
        if settings_path is None:
            # default settings path in AppData SafePass tree on Windows
            appdata = os.getenv('APPDATA') or os.getenv('LOCALAPPDATA')
            if not appdata:
                appdata = os.path.join(os.path.expanduser('~'), 'AppData', 'Roaming')
            settings_path = os.path.normpath(os.path.join(appdata, 'SafePass', 'settings.json'))
        ...
        # sanitize log directory (avoid empty/invalid path)
        try:
            safe_root = os.path.dirname(settings_path) if settings_path else os.getcwd()
            default_log_dir = os.path.join(safe_root, 'logs')
            if not isinstance(LOG_DIR, str) or not LOG_DIR.strip():
                LOG_DIR = default_log_dir
            else:
                LOG_DIR = LOG_DIR.strip()
                if not os.path.isabs(LOG_DIR):
                    LOG_DIR = os.path.normpath(os.path.join(safe_root, LOG_DIR))
        except Exception:
            appdata = os.getenv('APPDATA') or os.getenv('LOCALAPPDATA') or os.getcwd()
            LOG_DIR = os.path.normpath(os.path.join(appdata, 'SafePass', 'logs'))
    except Exception:
        pass
